...
def collate_fn_RNN_prfx(batch):
    ...
    # for i in x_seq:
    #     i = ut.keras_pad_sequences(i, padding='post')
    # for i in x_seq:
    #     i = ut.keras_pad_sequences(i, padding='post')
    # for i in x_seq:
    #     i = ut.keras_pad_sequences(i, padding='post')
...
def collate_fn_RNN_before(batch):
    x_seq, x_d, y = list(zip(*batch))
    x_len = [len(x) for x in x_seq]
    x_seq = ut.keras_pad_sequences(x_seq, padding='post')
...
def collate_fn_RNN(batch):
    x_seq, x_d, y = list(zip(*batch))
    x_len = [len(x[0]) for x in x_seq]
    x1 = []
    x2 = []
    for x in x_seq:
        x1=ut.keras_pad_sequences(x,maxlen=max(x_len), padding='post')
        x2.append(x1)
    x2=np.array(x2)
    x2_array = np.stack(x2).astype(np.int32)  
    # x_seq = ut.keras_pad_sequences(x_seq, padding='post')
    x2, x_d, x_len, y = torch.LongTensor(x2_array), torch.LongTensor(x_d), torch.LongTensor(x_len), torch.FloatTensor(y)
    return (x2, x_d, x_len), y
...
class DREAMEstimator(Estimator):
    ...
    def generate_substrings(self,query, num_substrings):
        ...
        while len(substrings)  < num_substrings:
            start = random.randint(0, int(query_len/2))  
            end = random.randint(start + 1, query_len)  
        ...
        # substrings.append(query)  
        random.shuffle(substrings)  
        return substrings

    def _encode_data_test(self, raw_data):
        if self.seq_out:
    ...
    def _encode_data(self, raw_data,num_substrings):
        if self.seq_out:
            ...
            if len(raw_data[0]) == 2:
                ...
                for rs in raw_data_bak:
                    ...
                    for d,c in enumerate(rs[1]):
                        ...
                        # substring_entries = [[ut.string_query_encoding(substr, self.char_dict), time_step, -1] for substr in substrings]
                        # original_entry = [ut.string_query_encoding(query, self.char_dict), time_step, result]
                        # raw_data.append([original_entry] + substring_entries)
                    # for d, c in enumerate(rs[1]):
                    #     raw_data.append([rs[0], d, c])
            # x_train = []
            # qd_list=[]
            # y_train=[]
            # for i in raw_data:
            #     data_list = list(zip(*i))
            #     x_train.append(data_list[0])
            #     qd_list.append(data_list[1])
            #     y_train.append(data_list[2])
            ...
            for q_s, q_d in zip(qs_list, qd_list):
                x=[]
                for q in q_s:
                    x.append(ut.string_query_encoding(q, self.char_dict))
                x_train.append(x)
    ...
    def dataloader(self, x_data, y_data, shuffle,train=False):
        bs = self.conf.bs
        assert bs >= 1
        ...
        if prfx_train:
            ...
            if train:
                ...
                dl_data = DataLoader(ds_data, batch_size=bs, shuffle=shuffle, collate_fn=collate_fn_RNN_before)
        return dl_data
    ...
    def train(self, train_data,loss_coefficient, num_substrings,valid_data=None, test_data=None):
        ...
        if valid_data:
            ...
            x_valid, y_valid = self._encode_data_test(valid_data)
        if test_data:
            # data_test = self._encode_data(test_data)
            x_test, y_test = self._encode_data_test(test_data)
        ...
        dl_train = self.dataloader(x_train, y_train, shuffle=True,train=True)
        x_train_len = []
        ...
        for epoch in range(1, max_epoch + 1):
            ...
            for batch in dl_train:
                ...
                x_batch, y_batch = self._device_batch(batch)
                pred_batch,_ = model.forward(x_batch, prfx_train=self.seq_out)
    ...
    def _average_loss(self, data_or_loader,num_substrings):
        ...
        for batch in dl_data:
            ...
            x_batch, y_batch = self._device_batch(batch)
            l_data = len(y_batch)
            loss_batch = model.loss_estimate(x_batch, y_batch).item()
    ...
    def estimate(self, data_or_loader):
        ...
        for batch in dl_data:
            ...
            pred_batch = model.forward_estimate(x_batch, prfx_train=prfx_train)
...
class RNN_module(nn.Module):
    ...
    def logit(self, x, hd=None, prfx_train=False):
        datas, delta, lengths = x
        splitted_data=torch.unbind(datas, dim=1)
        hiddens=[]
        for data in splitted_data:
            embed = self.embedding(data, delta)
            packed = pack_padded_sequence(embed, lengths.cpu(), batch_first=True, enforce_sorted=False)
            # rnn = self.rnn
            output, hidden = self.rnn(packed, hx=hd)
            if self.seq_out:
        ...
        loss_ord=0
        for i in range(1,datas.shape[1]):
            for j in range(datas.shape[0]):
                loss_ord = torch.sum(torch.max(torch.zeros_like(hiddens[0][0]),hiddens[i][j]-hiddens[0][j])**2)+loss_ord

        return output,loss_ord
        ...
        #     out = layer(out)
        # return out
    def logit_estimate(self, x, hidden=None, prfx_train=False):
    ...
    def forward(self, x, hidden=None, prfx_train=False):
        out,loss_ord = self.logit(x, prfx_train=prfx_train)
        out = F.leaky_relu(out)
        out = torch.squeeze(out)
        return out,loss_ord
    
    def forward_estimate(self, x, hidden=None, prfx_train=False):
        out = self.logit_estimate(x, prfx_train=prfx_train)
        out = F.leaky_relu(out)
        out = torch.squeeze(out)
        return out

    def loss_estimate(self, x, y):
        # assert isinstance(y, torch.Tensor)
        # prfx_train = (self.prfx is True) and y[0].dim() > 0
        seq_train = (self.seq_out is True) and y[0].dim() > 0
        pred_y = self.forward_estimate(x, prfx_train=seq_train)
        # if self.prfx:
    ...
    def loss(self, x, y,loss_coefficient):
        ...
        # prfx_train = (self.prfx is True) and y[0].dim() > 0
        seq_train = (self.seq_out is True) and y[0].dim() > 0
        pred_y,loss2 = self.forward(x, prfx_train=seq_train)
        # if self.prfx:
        #     y = y.reshape(-1, y.size()[-1])
        # print("pred:", pred_y)
        # exit()
